...
class PointCloudDataset(Dataset):
    def __init__(self, file_path, channel='Y'):
    ...
    def load_point_clouds(self):
        ...
        # Convert RGB to YUV
        yuv_colors = self.rgb_to_yuv(colors)
        
        # Select the specified channel
        if self.channel == 'Y':
            selected_channel = yuv_colors[:, 0:1]
        elif self.channel == 'U':
            selected_channel = yuv_colors[:, 1:2]
        elif self.channel == 'V':
            selected_channel = yuv_colors[:, 2:3]
        else:
            raise ValueError(f"Unknown channel: {self.channel}")
        
        # Combine points with the selected color channel
        point_cloud_data = np.hstack((points, selected_channel))
        self.point_clouds.append(point_cloud_data)
